...
async function getTemoignages(): Promise<Temoignage[]> {
  try {
    ...
    const res = await fetch(`${protocol}://${host}/api/temoignages`, {
      cache: "no-store",
    });
...
export default async function TemoignagesPage() {
  ...
  return (
    <main className="hf-page">
      ...
      <section className="hf-wrap-sm" style={{ textAlign: "center", paddingBottom: 40 }}>
      ...
      <section className="hf-section" style={{ paddingTop: 20 }}>
        <div className="hf-card hf-cta" style={{ maxWidth: 1000, margin: "0 auto" }}>
          <h2 className="hf-cta-title" style={{ fontSize: "clamp(30px,7vw,42px)" }}>
            Et si le prochain témoignage était le vôtre ?
          </h2>
          <p className="hf-cta-text" style={{ maxWidth: 700, margin: "0 auto 35px" }}>
